...
docker exec $DB_CONTAINER_1 mysqldump --user="DB1_User" --password="$DB1_Password" $DB_NAME_1 > $DB_DIR_1/$DB_NAME_1-$DATE.sql
gzip $DB_DIR_1/$DB_NAME_1-$DATE.sql
docker exec $DB_CONTAINER_2 mysqldump --user="DB2_User" --password="$DB2_Password" $DB_NAME_2 > $DB_DIR_2/$DB_NAME_2-$DATE.sql
gzip $DB_DIR_2/$DB_NAME_2-$DATE.sql
 if [ $? -eq 0 ]; then
